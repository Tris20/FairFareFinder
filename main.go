...
func filterRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	allFlightPrices, err := executeFlightPricesHistogramQuery(input)
	if err != nil {
		backend.HandleHTTPError(w, "Error executing all prices query", http.StatusInternalServerError)
		return
	}
...
func executeAccommodationPricesHistogramQuery(input *backend.FilterInput) ([]float64, error) {
	...
	allPricesQuery, allPricesArgs := buildQuery(input.LogicalExpression, backend.MaxAccomPrice, input.Cities, input.OrderClause)
...
func executeFlightPricesHistogramQuery(input *backend.FilterInput) ([]float64, error) {
	if db == nil {
		return nil, fmt.Errorf("database connection is not initialized")
	}
