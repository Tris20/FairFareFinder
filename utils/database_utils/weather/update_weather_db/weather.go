...
func storeWeatherData(dbPath string, airport AirportInfo, weatherData []WeatherData) error {
	...
	defer db.Close()
	 ...
	 for _, wd := range weatherData {
        ...
        var exists bool
        err := db.QueryRow(`SELECT EXISTS(SELECT 1 FROM weather WHERE city_name = ? AND country_code = ? AND date = ?)`,
        ...
        if exists {
            ...
            _, err = db.Exec(`UPDATE weather SET weather_type = ?, temperature = ?, weather_icon_url = ?, google_weather_link = ? WHERE city_name = ? AND country_code = ? AND date = ?`,
            ...
            _, err = db.Exec(`INSERT INTO weather (city_name, country_code, date, weather_type, temperature, weather_icon_url, google_weather_link) VALUES (?, ?, ?, ?, ?, ?, ?)`,
