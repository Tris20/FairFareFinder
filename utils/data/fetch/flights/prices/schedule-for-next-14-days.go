...
import (
	"database/sql"
	"fmt"
	"github.com/Tris20/FairFareFinder/utils/common/model" //import types
...
func DetermineFlightsFromConfig(origin model.OriginInfo) []model.DestinationInfo {
	...
	airports_db, err := sql.Open("sqlite3", "../../../../../data/raw/locations/locations.db")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
	...
	airportDetailsList := buildAirportDetails(airports_db, intersection)
...
func fetchAirportDetails(db *sql.DB, iataCode string) (string, string, string, error) {
	...
	query := "SELECT city, country, skyscannerid FROM airport WHERE iata = ?"
