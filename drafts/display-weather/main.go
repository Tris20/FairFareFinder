...
fmt.Println(apiKey)
		str_get := fmt.Sprintf("http://api.openweathermap.org/data/2.5/forecast?q=London&units=metric&appid=%s", apiKey)
...
log.Println("API response:", weatherResponse)
    ...
    for _, item := range weatherResponse.List[:forecastCount] {
        iconURL := fmt.Sprintf("http://openweathermap.org/img/wn/%s.png", item.Weather[0].Icon)
